const validator = {
  path: {
    source: [(v) => !!v || '출발지가 필요합니다.', (v) => v >= 0 || '출발지 ID는 0 이상의 양수이어야 합니다.'],
    target: [(v) => !!v || '목적지가 필요합니다.', (v) => v >= 0 || '목적지 ID는 0 이상의 양수이어야 합니다.']
  },
  departureTime: {
    dayTime: [(v) => !!v || '오전/오후 입력이 필요합니다.', (v) => !(v === '오전' || v === '오후') || '오전/오후로 입력해야 합니다.'],
    hour: [(v) => !!v || '시간 입력이 필요합니다.', (v) => (Number(v) >= 1 && Number(v) <= 12) || '올바른 시간 범위가 아닙니다.'],
    minute: [(v) => !!v || '분 입력이 필요합니다.', (v) => (Number(v) >= 0 && Number(v) <= 59) || '올바른 분 범위가 아닙니다.']
  },
